...
public enum AlertishType {
  case caution
  case success
  case warning
}

public class AlertishView: UIView {
  ...
  //Delegate
  public weak var delegate: AlertishViewDelegate?
  
  //AlertView Title & Subtitle Text
  var title: String?
  var subTitle: String = "You need to have a title or subtitle to use AlertishView"
  ...
  public convenience init(type: AlertishType){
    let result = UIScreen.main.bounds.size
    let frame = CGRect(x: 0, y: 0, width: result.width, height: result.height)
    self.init(frame: frame)
    switch type {
  ...
  private func renderCircleCutout(withAlertViewFrame alertViewFrame: CGRect){
    ...
    let rectPath = UIBezierPath(roundedRect: CGRect(x: 0,
                                                    ...
                                                    height: alertView!.frame.size.height), cornerRadius: 0)
  ...
  override public func draw(_ rect: CGRect) {
    ...
    if(!hideAllButtons && !hideDoneButton && numberOfButtons == 0) {
      ...
      otherButton.addTarget(self, action: #selector(handleButton), for: .touchUpInside)
      ...
      if hideDoneButton {
        firstButton.frame = CGRect(x: 0, y:alertViewFrame.size.height - 45, width: alertViewFrame.size.width/2, height: 45)
      } else {
        firstButton.frame = CGRect(x: 0, y: alertViewFrame.size.height - 135,  width: alertViewFrame.size.width, height:45)
      }
      
      firstButton.setTitle(buttonTitles![0], for: .normal)
      firstButton.addTarget(self, action: #selector(handleButton), for: .touchUpInside)
      ...
      secondButton.addTarget(self, action: #selector(handleButton), for: .touchUpInside)
      ...
      let firstSeparator = UIView(frame: CGRect(x: 0, y: firstButton.frame.origin.y - 2, width: alertViewFrame.size.width, height: 2))
      ...
      if !hideDoneButton {
        secondButton.frame = CGRect(x:0, y: alertViewFrame.size.height - 90, width: alertViewFrame.size.width, height: 45)
        secondSeparator.frame = CGRect(x: 0, y: secondButton.frame.origin.y - 2, width: alertViewFrame.size.width, height: 2)
        let doneButton = UIButton(type: .system)
        ...
        doneButton.frame = CGRect(x: 0, y: alertViewFrame.size.height - 45, width: alertViewFrame.size.width, height: 45)
        doneButton.setTitle(doneTitle, for: .normal)
        doneButton.addTarget(self, action: #selector(donePressed), for: .touchUpInside)
        doneButton.titleLabel?.font = UIFont.systemFont(ofSize: 18, weight: UIFontWeightMedium)
        alertView!.addSubview(doneButton)
        ...
        secondButton.frame = CGRect(x: alertViewFrame.size.width/2, y: alertViewFrame.size.height - 45, width: alertViewFrame.size.width/2, height: 45)
        secondSeparator.frame = CGRect(x: alertViewFrame.size.width/2 - 1, y: secondButton.frame.origin.y, width: 2, height: 45)
      }
    ...
    alertViewVector.frame = CGRect(x: alertViewContents!.frame.size.width/2 - 15.0, y: -15.0, width: 30.0, height: 30.0)
...
//    if let path = Bundle(forClass: AlertishView.self).path(forResource: "close-round", ofType: "png") {
//      setTheme(iconPath: path, tintColor: .flatRed)
//    }
  }
  
  private func makeAlertTypeCaution() {
//    if let path = Bundle(forClass: AlertishView.self).pathForResource("alert-round", ofType: "png") {
//      setTheme(iconPath: path, tintColor: .flatOrange)
//    }
  }
  
  private func makeAlertTypeSuccess(){
//    if let path = Bundle(forClass: AlertishView.self).path(forResource: "checkmark-round", ofType: "png") {
...
//    }
  ...
  func showAlertView() {
    ...
    delegate.alertishWillAppear(alertView: self)
  ...
  public func dismissAlertView() {
    UIView.animate(withDuration: 0.175, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseInOut, animations: {
      ...
      delegate.alertishDismissed(alertView: self)
  ...
  @objc private func donePressed(sender: UIButton){
    guard let delegate = delegate else { return }
    delegate.alertishDoneButtonClicked(alertView: self)
    self.dismissAlertView()
  }
}
